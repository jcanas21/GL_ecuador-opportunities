...
# baja el grueso de los puntos y reserva el rango alto para los mercados grandes.
PS_LOG_LO, PS_LOG_HI, PS_GAMMA, PS_R_MIN, PS_R_MAX = 7.0, 12.0, 2.0, 2.0, 15.0


def product_space_marker_size(mercado_accesible) -> pd.Series:
    """Diámetro del punto en píxeles a partir del mercado accesible en dólares."""
    v = pd.to_numeric(mercado_accesible, errors="coerce").fillna(0.0).clip(lower=1.0)
    t = np.clip((np.log10(v) - PS_LOG_LO) / (PS_LOG_HI - PS_LOG_LO), 0.0, 1.0) ** PS_GAMMA
    return pd.Series(PS_R_MIN + (PS_R_MAX - PS_R_MIN) * t, index=v.index) * 2.0
...
def load_product_space_layout() -> pd.DataFrame:
    ...
    ps = ps.merge(metricas[["hs4", "accessible_market_size"]], on="hs4", how="left")
